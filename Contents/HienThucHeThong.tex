...
\texttt{LoginActivity} là launcher của RobotApp. Sau xác thực, \texttt{MainActivity} quản lý các fragment hồ sơ, theo dõi, chatbot và cuộc gọi. Activity cũng lắng nghe \texttt{webrtc\_signal} để chuyển giao diện khi có lời mời gọi. \texttt{ReminderService} thực hiện việc theo dõi lịch nhắc nhở, còn \texttt{TrackingService} duy trì thông báo foreground và làm kênh chuyển sự kiện té ngã giữa khối theo dõi với chatbot. Việc thu nhận camera và suy luận AI hiện vẫn được thực hiện trong \texttt{tracking\_fragment}, chưa chuyển vào service. AndroidManifest khai báo camera, microphone, Internet, notification, foreground service và quyền BLE tương thích nhiều phiên bản Android.
...
Auto-follow ước lượng độ gần của mục tiêu từ kích thước vùng bao và khoảng cách giữa các mốc vai, hông, gối, mắt cá chân. Giá trị sau khi làm mượt được phân thành ba vùng quá xa, phù hợp và quá gần; các ngưỡng đi vào và rời khỏi mỗi vùng được đặt khác nhau để tránh robot đổi liên tục giữa tiến, dừng và lùi khi giá trị dao động gần biên. Thành phần tịnh tiến không còn là một mức cố định mà được \texttt{TrackingMotionPolicy} điều chỉnh theo khoảng cách: tốc độ tiến tăng dần trong khoảng từ 0,60 đến 0,90; tốc độ lùi khi mục tiêu ở quá gần thay đổi từ 0,45 đến 0,60. Khi cần lùi để quan sát lại phần chân, hệ thống sử dụng mức 0,30. Nếu robot vừa quay vừa tiến hoặc chưa quan sát ổn định mắt cá chân, tốc độ tiến được giảm để hạn chế mất mục tiêu. Sai lệch ngang giữa tâm vùng bao và tâm ảnh tạo thành phần quay liên tục, có vùng chết và giới hạn biên để giảm dao động.
...
Manager quét theo tên \texttt{OhmniRobot} và custom UUID, timeout quét 10 s, reconnect sau 3 s. Sau discovery, characteristic được cấu hình \texttt{WRITE\_TYPE\_NO\_RESPONSE}. API ghi khác nhau theo phiên bản Android được xử lý tương thích. Nếu luồng RTDB không có lệnh mới trong 500 ms, bộ định thời an toàn gửi lệnh dừng xuống ESP32. Hàm \texttt{stop()} cũng gửi lệnh dừng trước khi gỡ listener, callback, tiến trình quét và kết nối GATT.
...
\texttt{ScheduleFragment} đọc danh sách \texttt{Schedules/uid}; \texttt{AddScheduleFragment} tạo hoặc cập nhật lịch nhắc nhở; \texttt{ScheduleAdapter} xử lý RecyclerView và công tắc \texttt{active}. Tên trường được giữ đúng là \texttt{active}, tránh sai lệch với \texttt{isActive}. Ba nhánh nhật ký có fragment/adaptor riêng: reminder, fall alert và call history. Danh sách cảnh báo/cuộc gọi được truy vấn theo timestamp trong bảy ngày gần nhất.
