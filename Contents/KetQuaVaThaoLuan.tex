...
\texttt{FallVerificationEngineTest} định nghĩa bốn nhánh quan trọng. Engine trước hết được ``arm'' bằng năm frame đứng và một raw fall, sau đó đánh giá tình huống.
...
\hline
clipped bbox after fall & Thân thấp, bbox không nằm trọn ảnh & Hỏi ngay \\
\hline
...
Đối chiếu hai ứng dụng cho thấy các path quan trọng trùng nhau: \path|robot_control|, \path|robot_follow_enabled|, \path|ai_fall_detection_enabled|, \path|webrtc_signal|, \path|fall_alerts|, \path|call_history|, \texttt{Users}, \texttt{Schedules} và \path|reminder_logs|. Offer/answer dùng cùng trường \texttt{sdp,type}. ICE dùng \path|candidate,sdpMid,sdpMLineIndex|; hai nhánh candidate được tách theo vai trò.
...
Để đáp ứng yêu cầu cảnh báo thời gian thực, toàn bộ pipeline (Decode Video $\rightarrow$ YOLO Pose $\rightarrow$ LSTM) phải chạy trực tiếp trên thiết bị Android với tốc độ khung hình (FPS) tối thiểu từ 10 đến 15 FPS. Benchmark tốc độ suy luận được thực hiện đo lường (profiling) trên nền tảng TFLite.

\begin{table}[htbp]
\centering
\caption{Thời gian suy luận (Inference Time) trung bình trên Android}
\label{tab:speed_benchmark}
\begin{tabular}{|l|c|c|}
\hline
\textbf{Thành phần AI} & \textbf{Model Artifact} & \textbf{Thời gian xử lý / Frame (ms)} \\
\hline
...
\hline
\textbf{Tổng cộng (End-to-End)} & \textbf{Toàn bộ Pipeline} & \textbf{$\sim 60$ ms (tương đương $\sim 16$ FPS)} \\
\hline
\end{tabular}
\end{table}

Như trình bày ở Bảng \ref{tab:speed_benchmark}, nhờ việc lượng tử hóa YOLO sang định dạng FP16 và tối ưu hóa graph, thời gian xử lý toàn bộ một frame chỉ mất khoảng 60 ms. Với mức hiệu năng $\sim 16$ FPS, hệ thống hoàn toàn đảm bảo khả năng theo dõi liên tục và phát hiện cú ngã ngay trong khoảnh khắc nó đang diễn ra mà không bị trễ hình (lag).
...
Thứ nhất, chưa có bảng metric huấn luyện/validation và benchmark thiết bị được lưu cùng artifact; vì vậy không thể đưa ra mAP, PCK, precision/recall, FPS hay latency đáng tin cậy. Thứ hai, dữ liệu té ngã dựng lại khác người cao tuổi thật; domain shift có thể lớn. Thứ ba, camera RGB gây vấn đề riêng tư và có điểm mù. Thứ tư, Groq/Firebase/TURN là phụ thuộc ngoài; mất Internet làm suy giảm chức năng. Thứ năm, node toàn cục và auth/rules cần hoàn thiện cho multi-tenant. Thứ sáu, APK RobotApp debug còn quá lớn.
...
Nguyên mẫu đã hiện thực đầy đủ các khối kiến trúc và tạo được hai ứng dụng đóng gói, model triển khai cùng firmware. Kiểm tra tĩnh xác nhận sự nhất quán của cấu trúc dữ liệu Firebase, giao thức BLE và các lớp bảo vệ. Bộ unit test hiện có tập trung vào dấu điều khiển và hậu kiểm té ngã. Tuy nhiên, báo cáo không khẳng định độ chính xác hay độ trễ khi chưa có log benchmark; đây là giới hạn quan trọng và cũng là kế hoạch đánh giá tiếp theo. Chương cuối tổng kết đóng góp và hướng phát triển từ các kết quả này.
